...
{
    av_frame_unref(vp->frame); //清空缓存用的
...
static void video_refresh(FFPlayer *opaque, double *remaining_time)
{
    ...
    if (!ffp->display_disable && is->show_mode != SHOW_MODE_VIDEO && is->audio_st) {
        printf("show_mode != SHOW_MODE_VIDEO");
...
display:
            ...
            if (!ffp->display_disable && is->show_mode == SHOW_MODE_VIDEO)
                video_display2(ffp);//这个是视频输出!!!!

            //peek 是检查的 这个是移动到一下frame,减少size
            frame_queue_next(&is->pictq);
            SDL_LockMutex(ffp->is->play_mutex);
            
            printf("is->step: %d",is->step);
            if (is->step) { //playing is not 0  看不懂这个是控制什么的
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    if (!is)
        return NULL;
    av_strlcpy(is->filename, filename, sizeof(is->filename)); //复制字符串,先别管这玩意干嘛
    is->iformat = iformat;
    is->ytop    = 0;
    is->xleft   = 0;
...
//SDL_CreateThreadEx 创建线程,执行方法
    is->video_refresh_tid = SDL_CreateThreadEx(&is->_video_refresh_tid, video_refresh_thread, ffp, "ff_vout");//视频输出流
    if (!is->video_refresh_tid) {
        av_freep(&ffp->is);
        return NULL;
    }

    is->read_tid = SDL_CreateThreadEx(&is->_read_tid, read_thread, ffp, "ff_read"); //创建读取流的指针,第二个参数是执行方法的名称
    if (!is->read_tid) {
fail:
        is->abort_request = true;
        if (is->video_refresh_tid) //视频读取完毕,等待输出流完成
            SDL_WaitThread(is->video_refresh_tid, NULL);
        stream_close(is);
        return NULL;
    }
