...
static int ijkmp_start_l(IjkMediaPlayer *mp)
{
    ...
    MP_RET_IF_FAILED(ikjmp_chkst_start_l(mp->mp_state)); //不知道MP_RET_IF_FAILED这个函数干嘛的

    ffp_remove_msg(mp->ffplayer, FFP_REQ_START); //移除开始状态
    ffp_remove_msg(mp->ffplayer, FFP_REQ_PAUSE); //移除暂停状态
    ffp_notify_msg1(mp->ffplayer, FFP_REQ_START); //开始状态
...
int ijkmp_start(IjkMediaPlayer *mp)
{
    assert(mp);
    MPTRACE("ijkmp_start()\n");
    pthread_mutex_lock(&mp->mutex); //上锁
    int retval = ijkmp_start_l(mp); //retval 是否打开成功
    pthread_mutex_unlock(&mp->mutex);
