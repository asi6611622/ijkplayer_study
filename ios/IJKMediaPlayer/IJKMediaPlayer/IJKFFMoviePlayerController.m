           ...
           withSegmentResolver:(id<IJKMediaSegmentResolver>)segmentResolver
{
    ...
    if (self) {
        ...
        //media_player_msg_loop 开始捕获列队消息 whil(ffcontroller)
        //
        _msgPool = [[IJKFFMoviePlayerMessagePool alloc] init]; //大概就是弹幕?

        ijkmp_set_weak_thiz(_mediaPlayer, (__bridge_retained void *) self); //把刚刚创建的_mediaPlayer 赋值到 当前的mp 算是把对象刷新一下
        ijkmp_set_format_callback(_mediaPlayer, format_control_message, (__bridge void *) self); //format 什么鬼
        ijkmp_set_option_int(_mediaPlayer, IJKMP_OPT_CATEGORY_PLAYER, "start-on-prepared", _shouldAutoplay ? 1 : 0);

        // init video sink
        _glView = [[IJKSDLGLView alloc] initWithFrame:[[UIScreen mainScreen] bounds]];//这个播放器的皮
        _view   = _glView;
...
#endif
        // init audio sink
        [[IJKAudioKit sharedInstance] setupAudioSession]; //这个是音频
        ...
        // init extra
        _keepScreenOnWhilePlaying = YES; //播放时保持高亮
        [self setScreenOn:YES];

        _registeredNotifications = [[NSMutableArray alloc] init];
        [self registerApplicationObservers]; //监听系统声音变化状态
    }
    return self;
}
...
- (void)prepareToPlay
{
    ...
    //这里已经开始读取流了!!! 里面会检查_mediaPlayer的datasource
    ijkmp_prepare_async(_mediaPlayer);//锁住mp 检查播放器运行的状态mp_stat,并标记mp_stat为MP_STATE_INITIALIZED,
}
...
- (void)play
{
    ...
    ijkmp_start(_mediaPlayer); //通知可以开始loop
}
